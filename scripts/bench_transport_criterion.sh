...
write_summary() {
    ...
    cat >"$summary" <<SUMMARY
# Zenoh Payload-Contract Representative Benchmarks

## Command

\`\`\`bash
TRANSPORT_BENCH_SUITE=$TRANSPORT_BENCH_SUITE TRANSPORT_BENCH_PROFILE=$TRANSPORT_BENCH_PROFILE cargo bench --features $features --bench transport_criterion -- $CRITERION_ARGS
\`\`\`
...
This script is the Phase 06C2 authority wrapper for the 06C1 representative-v1 command shape. It writes artifacts only under the caller-selected report directory.
SUMMARY
}
