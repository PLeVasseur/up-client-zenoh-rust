...
use up_rust::{
    ...
    LocalUriProvider, PayloadEncoding, UListener, UMessage, UMessageBuilder, UPriority, UTransport,
    UUri, UUID,
};
...
impl UListener for RequestListener {
    async fn on_receive(&self, msg: UMessage) {
        ...
        let umessage = UMessageBuilder::response_for_request(&attributes)
            .build_with_payload(self.response_data.clone(), PayloadEncoding::TEXT)
            .unwrap();
        task::block_in_place(|| {
            Handle::current()
...
async fn test_rpc_server_client(
    ...
    let uptransport_client = Arc::new(
        test_lib::create_up_transport_zenoh(src_uuri, None)
            .await
            .unwrap(),
    );
    let uptransport_server = Arc::new(
        test_lib::create_up_transport_zenoh(sink_uuri, None)
            .await
            .unwrap(),
    );
    ...
    // Send Request with ZenohRpcClient (L2 API)
    {
        ...
        let payload = UPayload::new(request_data.clone(), PayloadEncoding::TEXT);
    ...
    // Send Request with Transport Layer (L1 API)
    {
        ...
        let umessage = UMessageBuilder::request(sink_uuri.clone(), src_uuri.clone(), 1000)
            .build_with_payload(request_data.clone(), PayloadEncoding::TEXT)
            .unwrap();
