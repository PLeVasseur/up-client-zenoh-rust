...
use up_rust::{
    ...
    LocalUriProvider, PayloadEncoding,
};
...
impl RequestHandler for ExampleHandler {
    async fn handle_request(
        ...
        // return
        let payload = UPayload::new(self.response_data.clone(), PayloadEncoding::TEXT);
        Ok(Some(payload))
    }
}
...
async fn test_l2_rpc() {
    ...
    let uptransport_server = Arc::new(
        test_lib::create_up_transport_zenoh("//l2_responder/2/1/0", None)
            .await
            .unwrap(),
    );
    let uptransport_client = Arc::new(
        test_lib::create_up_transport_zenoh("//l2_requester/1/1/0", None)
            .await
            .unwrap(),
    );
    ...
    let payload = UPayload::new(request_data, PayloadEncoding::TEXT);
