...
use up_rust::{
    LocalUriProvider, PayloadEncoding, UListener, UMessage, UMessageBuilder, UMessageType,
    UTransport, UUri,
};
        ...
        match attributes.type_() {
            ...
            UMessageType::Request => {
                let umessage = UMessageBuilder::response_for_request(&attributes)
                    .build_with_payload(self.predefined_resp_data.clone(), PayloadEncoding::TEXT)
                    .unwrap();
                task::block_in_place(|| {
                    Handle::current()
...
async fn test_ustreamer() {
    ...
    // Initialization
    let uclient = Arc::new(
        test_lib::create_up_transport_zenoh("//uclient/1/1/0", None)
            .await
            .unwrap(),
    );
    let ustreamer = Arc::new(
        test_lib::create_up_transport_zenoh("//ustreamer/2/1/0", None)
            .await
            .unwrap(),
    );
    ...
    // Send Request (uclient => uStreamer)
    {
        ...
        let umessage = UMessageBuilder::request(sink_uuri.clone(), src_uuri.clone(), 1000)
            .build_with_payload(String::from("Not matter"), PayloadEncoding::TEXT)
            .unwrap();
        uclient.send(umessage).await.unwrap();
