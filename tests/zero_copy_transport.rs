...
use tokio::sync::mpsc;
use up_rust::{
    NativePrefixFrameMetadataCodec, PayloadCodecIdentity, PayloadEncoding, PayloadLoanProvenance,
    ProtobufWire, UCode, UEncodedLoanedRxFrame, UFrameMetadata, UFrameView, UProtocolNativeWire,
    UTxBuffer, UTxLoanSpec, UUninitTxBuffer, UUri, UWire, UWireMetadataCodec, UWireRx,
    UZeroCopyListener, UZeroCopyTransportImpl, UZeroCopyUninitTransportImpl, WireIdentity,
    NATIVE_EXPLICIT_PAYLOAD_FAMILY_ID, PROTOBUF_WIRE_ID, UFRAME_FIELDS_METADATA_LAYOUT_ID,
};
...
fn zenoh_key(source: &UUri, sink: Option<&UUri>) -> String {
    fn part(uri: &UUri) -> String {
        ...
        format!(
            "{}/{ue_type}/{ue_instance}/{version}/{resource}",
            uri.authority_name()
        )
    }
    ...
    W: UWire + Default + Send + Sync + 'static,
{
    ...
    let mut tx = transport.loan_validated_tx(spec).await?;
...
async fn receive_zero_copy_returns_shm_payload_lease() -> Result<(), TestError> {
    ...
    let receive_task = tokio::spawn(async move {
        receiver
            .receive_validated_zero_copy(&receive_source, None)
            .await
    });
    allow_subscriber_matching().await;
    ...
    let mut buffer = transport
        .loan_validated_tx(UTxLoanSpec::payload(metadata.clone(), payload.len(), 1)?)
        .await?;
...
async fn zero_copy_listener_fanout_delivers_rx_leases() -> Result<(), TestError> {
    ...
    let mut buffer = transport
        .loan_validated_tx(UTxLoanSpec::payload(second_metadata, payload.len(), 1)?)
        .await?;
    buffer.payload_mut().copy_from_slice(payload);
    transport.send_validated_zero_copy(buffer).await?;
...
async fn zero_copy_unregister_listener_removes_exact_and_wildcard_listeners(
    ...
    transport
        .register_validated_zero_copy_listener(&source, None, exact_listener.clone())
        .await?;
    transport
        .register_validated_zero_copy_listener(&wildcard, None, wildcard_listener.clone())
        .await?;
    allow_subscriber_matching().await;

    transport
        .unregister_validated_zero_copy_listener(&source, None, exact_listener)
        .await?;
    allow_subscriber_matching().await;
    let payload = b"wildcard-only";
    let first_metadata = metadata(source.clone(), Some(PayloadEncoding::PROTOBUF));
    let mut buffer = transport
        .loan_validated_tx(UTxLoanSpec::payload(first_metadata, payload.len(), 1)?)
        .await?;
    buffer.payload_mut().copy_from_slice(payload);
    transport.send_validated_zero_copy(buffer).await?;
    ...
    transport
        .unregister_validated_zero_copy_listener(&wildcard, None, wildcard_listener)
        .await?;
    allow_subscriber_matching().await;
...
async fn zero_copy_uninit_transmit_uses_selected_wire_metadata() -> Result<(), TestError> {
    ...
    let metadata = metadata(source, Some(PayloadEncoding::PROTOBUF));
    let mut buffer = transport
        .loan_validated_uninit_tx(UTxLoanSpec::payload(metadata.clone(), payload.len(), 1)?)
        .await?;
...
async fn zero_copy_wrong_wire_metadata_is_rejected_before_pull_receive_exposes_frame(
    ...
    let mut buffer = sender
        .loan_validated_tx(UTxLoanSpec::payload(metadata, payload.len(), 1)?)
        .await?;
    buffer.payload_mut().copy_from_slice(payload);
    sender.send_validated_zero_copy(buffer).await?;
...
async fn zero_copy_sink_filter_mismatch_is_not_delivered_to_pull_receive() -> Result<(), TestError>
{
    ...
    let receive_task = tokio::spawn(async move {
        receiver
            .receive_validated_zero_copy(&receive_source, Some(&receive_sink))
            .await
    });
    allow_subscriber_matching().await;
