...
use up_rust::{
    LocalUriProvider, PayloadEncoding, UListener, UMessage, UMessageBuilder, UTransport,
};
...
async fn test_blocking_user_callback() {
    ...
    // Initialization
    let uptransport_send = test_lib::create_up_transport_zenoh("//nonblock_pub/1/1/0", None)
        .await
        .unwrap();
    let uptransport_recv = test_lib::create_up_transport_zenoh("//nonblock_sub/2/1/0", None)
        .await
        .unwrap();
    ...
    let umsg0 = UMessageBuilder::publish(pub_uuri.clone())
        .build_with_payload("Pub 0", PayloadEncoding::TEXT)
        .unwrap();
    let umsg1 = UMessageBuilder::publish(pub_uuri.clone())
        .build_with_payload("Pub 1", PayloadEncoding::TEXT)
        .unwrap();
